...
function openInfo(tab) {
    const infoTexts = {
        main: `<b>Главная</b><br>Загрузите XML-файлы с коллизиями и ZIP-архивы с изображениями. После загрузки появится предпросмотр статистики, таблица пар категорий и кнопка запуска анализа. После анализа вы сможете скачать результаты и при необходимости выполнить ручную разметку спорных случаев.`,
        settings: `<b>Настройки</b><br>Выберите модель для инференса, настройте пороги уверенности (low/high confidence), формат экспорта и включите/отключите ручную разметку. Здесь отображаются метрики выбранной модели, матрица ошибок и обученные пары категорий.`,
        train: `<b>Обучение</b><br>Загрузите XML-файлы и ZIP-архивы с изображениями для обучения новой модели. После загрузки появится статистика по данным, настройте параметры обучения (batch size, epochs) и запустите процесс. Прогресс обучения и метрики будут отображаться в реальном времени.`,
        progress: `<b>Прогресс обучения</b><br>Следите за статусом обучения: графики метрик (loss, accuracy, F1-score), итоговые показатели и подробная статистика по каждому XML-файлу доступны в реальном времени. После завершения обучения модель будет доступна для выбора в настройках.`
    };
