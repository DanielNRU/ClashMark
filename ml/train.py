...
def collect_dataset_from_multiple_files(xml_paths, images_dir=None, export_format='standard'):
    """
    Собирает датасет из нескольких XML-файлов и сопоставляет изображения.
    Возвращает DataFrame только с валидными изображениями и нужными классами.
    """
    all_dataframes = []
    for xml_path in xml_paths:
        df = parse_xml_data(xml_path, export_format=export_format)
        if len(df) == 0 or not images_dir:
            continue
...
def create_transforms(is_training=True):
    ...
    Для обучения — с аугментациями, для инференса — только resize и to_tensor.
    """
    if is_training:
...
def train_model(
    df, epochs=10, batch_size=16, learning_rate=1e-4, device=None,
    progress_callback=None, model_filename=None, model_type='mobilenet_v3_small'):
    """
    Обучает модель на размеченных данных.
    - Делит данные на train/val.
    - Логирует метрики и сохраняет веса модели.
    - Поддерживает callback для отображения прогресса.
    """
    # Проверяем, что есть колонка IsResolved (0 — разрешено, 1 — активно)
    ...
    # Делим на train/val с сохранением баланса классов
    train_df, val_df = train_test_split(
        df, test_size=0.2, random_state=42, stratify=df['IsResolved']
    )
    ...
    # Балансировка классов для BCEWithLogitsLoss
    ...
    for epoch in range(epochs):
        ...
        if val_targets and val_preds and len(set(val_targets)) > 1:
            ...
            val_precision = None
            val_conf_matrix = None
        # Вызываем callback после валидации (раз в эпоху)
        if progress_callback:
            try:
                progress_callback(
    ...
    # Матрица ошибок для последней эпохи
    if val_targets and val_preds and len(set(val_targets)) > 1:
